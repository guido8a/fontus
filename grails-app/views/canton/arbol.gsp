            ...
            case "pais" :
                submenu = {
                    "provincia" : {
                        "label"  : "Provincia",
                        "action" : function (obj) {
                            %{--var url = "${createLink(controller: 'provincia', action: 'form_ajax')}";--}%
                            %{--$.ajax({--}%
                                %{--"type"    : "POST",--}%
                                %{--"url"     : url,--}%
                                %{--"data"    : {--}%
                                ...
                                %{--"success" : function (msg) {--}%
                                    %{--$("#dlg_editar").dialog("option", "title", "Crear una provincia");--}%
                                    %{--$("#dlg_editar").html(msg);--}%
        ...
        if(tipoNode == 'provincia'){
            ...
            items.eliminar = {
                ...
                "action"           : function (obj) {
                    ...
                    var parts = str.split("_");
                    var id = parts[1];
                    var mensaje = "Está seguro de querer eliminar esta provincia?\nEsta acción no se puede deshacer...";
...
//                        $("#frmDelete-provinciaInstance").submit();
                            $.ajax({
                               type: 'POST',
                                url: '${createLink(controller: 'provincia', action: 'delete')}',
                                data:{
                                    id: id
                                },
                                success: function (msg){
                    ...
                    $("#modalBody").html("<p>¿Está seguro de querer eliminar esta provincia?</p>");
...
//                                    reloadTree();
    ...
    function initTree() {
        $("#tree").jstree({
            ...
            open_parents  : false,
            "html_data"   : {
//                        "data" : "<ul type='provincia'><li id='provincia_1' class='provincia jstree-closed' rel='provincia'><a href='#' class='label_arbol'>Ecuador</a></ul>",
                "data" : "<ul type='pais'><li id='pais_1' class='pais jstree-closed' rel='pais'><a href='#' class='label_arbol'>Ecuador</a></ul>",
                "ajax" : {
