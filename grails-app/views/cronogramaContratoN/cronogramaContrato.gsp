...
<body>
%{--<g:set var="meses" value="${obra.obra?.plazoEjecucionMeses + (obra.obra?.plazoEjecucionDias > 0 ? 1 : 0)}"/>--}%
%{--<g:set var="plazoOk" value="${detalle.findAll { it.dias > 0 }.size() > 0}"/>--}%
%{--<g:set var="plazoOk" value="${paraPlazo.findAll { it.obraContrato?.obra?.plazoEjecucionDias > 0 }.size() > 0}"/>--}%
%{--<g:set var="matrizOk" value="${obra.desgloseTransporte != null}"/>--}%
%{--<g:set var="matrizOk" value="${tieneMatriz}"/>--}%
<g:set var="meses" value="${meses}"/>
...
<div class="btn-toolbar">
    <div class="btn-group">
        <a href="${g.createLink(controller: 'contrato', action: 'registroContrato', params: [contrato: obra?.contrato?.id])}" class="btn btn-ajax btn-new" id="atras" title="Regresar al contrato">
...
<script type="text/javascript">
    ...
    $("#subpres").change(function () {
        $.ajax({
            type    : "POST", url : "${g.createLink(controller: 'cronogramaContratoN', action:'cargarAreasContrato_ajax')}",
