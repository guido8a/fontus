...
class ObraMFController {
    ...
    def validaciones() {
        ...
        println "validaciones: $params"
        if (params.borraFP == "true") obraService.borrarFP(params.obra)
        def fin = new Date()
        println "...borra FP --> ${TimeCategory.minus(fin, inicio)}"
        res = pone_ids()
...
/*
        ...
        fin = new Date()
        println "...actualiza HR --> ${TimeCategory.minus(fin, inicio)}"
        ...
        fin = new Date()
        println "...sp_obra --> ${TimeCategory.minus(fin, inicio)}"
*/
        ...
        fin = new Date()
        println "...antes de verifica matriz --> ${TimeCategory.minus(fin, inicio)}"
        println "....4"
        ...
        fin = new Date()
        println "pasa validaciones --> ${TimeCategory.minus(fin, inicio)}"
//        println "pasa validaciones"
    ...
    def matrizFP() {
        ...
        pone_ids()
        def fin = new Date()
        println "...pone_ids --> ${TimeCategory.minus(fin, inicio)}"
        //def obra = Obra.get(obra__id)
...
//        println "verifica_precios \n" + verifica_precios(obra__id)
        ...
        * Verifica si existe Transporte y/o Equipos'                                          */
        ...
        if (conTransporte)
            ...
            transporte = equipos
        def hayEquipos = (transporte > 0.000001)
...
//        println "5. inicia crear campos"
        numeroCampos += creaCamposPg(obra__id, sbpr, hayEquipos);

        println "5. fin ... nuevo creaCampos $numeroCampos"
...
*/
        ...
        fin = new Date()
        println "...7. inicio inserta rubros --> ${TimeCategory.minus(fin, inicio)}"
//        println "7. inicio inserta rubros"
        ...
        fin = new Date()
        println "...8. fin inserta rubros --> ${TimeCategory.minus(fin, inicio)}"
//        println "8. fin inserta rubros"

//        render "*********"

        /* ---- ejecuta Rubros(subPrsp) y Descomposicion(subPrsp) ----------------------------- */
//        rubros(obra__id, sbpr)  //anterior
        mf_rubros(obra__id, sbpr)  //nuevo
        fin = new Date()
        println "...9. completa rubros --> ${TimeCategory.minus(fin, inicio)}"

//        descomposicion(obra__id, sbpr)
        mf_descomposicion(obra__id, sbpr) //nuevo
        fin = new Date()
        println "...10. completa descomposicion --> ${TimeCategory.minus(fin, inicio)}"

//        des_Materiales(obra__id, sbpr, conTransporte)
        mf_materiales(obra__id, sbpr, conTransporte)
        fin = new Date()
        println "...11. completa des_Materiales --> ${TimeCategory.minus(fin, inicio)}"
//        println "11. completa des_Materiales, conTranp: $conTransporte ... Hay equipos: $hayEquipos"
        if (hayEquipos) {
            println "ejecuta hayEquipos"
            if (conTransporte) acTransporte(obra__id, sbpr)
//            acEquipos(obra__id, sbpr)
            mf_equipos(obra__id, sbpr)
        }
        fin = new Date()
...
//        guardaSbpr(obra__id, sbpr)
    ...
    def verificaMatriz(id) {
        ...
        def errr = ""
        if (!VolumenesObra.findByObra(obra)) errr += "<br><span class='label-azul'>No se ha ingresado los volúmenes de Obra</span>"
    ...
    */
    def transporteEspecial(id) {
        ...
        println "inicia proceso...transporteEspecial  para obra: $id"
        def res
...
*/
    ...
    def calculaEquipos(id) {
        def cn = dbConnectionService.getConnection()
        def tx_sql = "select coalesce(sum(voitcntd), 0) equipos from vlobitem, item, dprt, sbgr "
...
//        ejecutaSQL("insert into mfcl values (${numeroCampos}, '${id}', '${campo}', '${tipo}', ${sbpr})")
    }
...
//        ejecutaSQL("update mfvl set valor = ${incr} where obra__id = ${id} and clmncdgo = ${clmn} and codigo = '${rbro}' " +
        ejecutaSQL("update mfvl set valor = ${pcun * cntd} where obra__id = ${id} and clmncdgo = ${clmn} and codigo = '${rbro}' " +
                "and sbpr__id = ${sbpr}")
            ...
            cn1.eachRow(tx_cr.toString()) { d ->
                if (d.pcun > 0) {
                    ...
                    ejecutaSQL("update mfvl set valor = ${d.pcun * cntd} where obra__id = ${id} and codigo = '${row.itemcdgo}' " +
                            " and clmncdgo = ${clmn} and sbpr__id = ${sbpr}")
...
//        def totalTrnp = (totalSx(id, 'TRANSPORTE_T', 'sS1') + totalSx(id, "${id_equipo}_T", "sS1"))
        ...
        println "transporte: $transporte, total equipos:" + totalSx(id, "${id_equipo}_T", "sS1", sbpr)
...
*/
    ...
    def creaCamposPg(obra__id, sbpr, hayEquipos) {
        def cn = dbConnectionService.getConnection()
        def cont = 0
        cn.eachRow("select * from mf_columnas($obra__id, $sbpr, $hayEquipos)".toString()) {
            cont = it.mf_columnas
        }
        cn.close()
        return cont
    ...
    def mf_rubros(obra__id, sbpr) {
        def cn = dbConnectionService.getConnection()
        def cont = 0
        cn.execute("select * from mf_rubros($obra__id, $sbpr)".toString())
        cn.close()
    }

    def mf_descomposicion(obra__id, sbpr) {
        def cn = dbConnectionService.getConnection()
        def cont = 0
        cn.execute("select * from mf_descomposicion($obra__id, $sbpr)".toString())
        cn.close()
    }

    def mf_materiales(obra__id, sbpr, conTransporte) {
        def cn = dbConnectionService.getConnection()
        def cont = 0
        cn.execute("select * from mf_desMateriales($obra__id, $sbpr, $conTransporte)".toString())
        cn.close()
    }

    def mf_equipos(obra__id, sbpr) {
        def cn = dbConnectionService.getConnection()
        def cont = 0
        cn.execute("select * from mf_equipos($obra__id, $sbpr)".toString())
        cn.close()
    }
